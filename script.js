...
const app = {
    ...
    init: async () => {
        // UI-Status: Erstmal Lade-Spinner zeigen
        document.getElementById('login-container').classList.add('hidden');
        document.getElementById('auth-loading').classList.remove('hidden');
        document.getElementById('auth-overlay').classList.remove('hidden');

        // WICHTIG: Persistenz auf LOCAL setzen, damit Safari sich den User merkt
        try {
        ...
        try {
            ...
            if (result.user) {
                ...
                // onAuthStateChanged feuert gleich automatisch, wir müssen hier nichts tun
            }
        } catch (error) {
    ...
    handleLoginSuccess: (user) => {
        ...
        // Strenge Prüfung: Nur Schule oder Admin
        if (!email.endsWith('@europagym.at') && email !== 'admin@europagym.at') { 
            console.warn("Unerlaubte Email:", email);
            auth.signOut(); 
        ...
        // Toast nur anzeigen, wenn wir nicht schon "drin" waren
    ...
    logout: () => {
        sessionStorage.removeItem('userEmail');
        // Wir laden die Seite NICHT neu, sondern loggen nur aus.
        // onAuthStateChanged kümmert sich um das UI.
        auth.signOut().then(() => {
            console.log("Ausgeloggt.");
            // Optional: location.reload() nur wenn unbedingt nötig
            // location.reload(); 
        }).catch(err => {
    ...
    adminLogin: async () => {
        ...
        try {
            ...
            // Wir müssen hier nichts weiter tun, onAuthStateChanged fängt den neuen User ab
            // und ruft handleLoginSuccess auf.
            
            document.getElementById('admin-auth-modal').classList.add('hidden');
            app.nav('admin');
            
            // Kleiner Hack: Wenn wir von User zu Admin wechseln, müssen wir sicherstellen,
            // dass die Listener neu starten. onAuthStateChanged macht das, aber wir müssen
            // app.listenersStarted zurücksetzen.
